...
<script>
    ...
    export default {
        ...
        mounted(){
          ...
          this._initBScroll()    //初始化滑动和动态获取ui的长度
        },
        data(){
          return{
        ...
        methods:{
          _initBScroll(){
            ...
            /*动态获取UI的宽度*/
            const UI = this.$refs.picsUI
            const widthPic = 120
            const marginRight = 6
            const count = this.info.pics.length
            UI.style.width = (widthPic + marginRight) * count - marginRight +"px"
        ...
        watch: {
          info () {//shopInfo页面更新分两步第一步为初始化页面，第二步为更新数据 而更新数据是异步更新 所以说如果初始化页面没有数据而直接调用initBScroll方法会直接出错
            this.$nextTick(() => {  //如果在当前的页面刷新时 info会更新数据 一旦更新数据则会调用this.initBScroll方法 因为数据已经有了
              this._initBScroll()
            })
          }
        }
